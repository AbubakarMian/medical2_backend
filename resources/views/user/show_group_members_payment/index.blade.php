...
<div class="bgareaa">
    ...
    <div class="container">
        ...
        <form role="form" method="post" action="{{action('User\CoursesController@group_payment_finalize')}}">
            ...
            <!--toggle -->
            <label class="switch">
                <input type="checkbox" class="highlight_class_{{$c_r->user_id}}{{$c->id}}" onclick="highlight_div('{{$c->id}}','{{$c_r->user_id}}','{{$c->amount}}')" name="student_id[]" value="{{$c->id}}">
    ...
    <script>
        ...
        function highlight_div(e, u, a) {
            ...
            if ($(highlight_class).is(':checked')) {
                ...
                var my_amount_date = $('.my_amount_date_' + user + group_fess_id).css("background-color", "");
                all_amount = parseFloat(all_amount) - parseFloat(a);
            }
