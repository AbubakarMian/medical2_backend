                    ...
                    <div class="container-fluid">
                            ...
                            <div class="col-sm-12">
                                <div class="camboclicks">
                                    <div class="form-group camboclick">
                                        <button type="button" onclick="open_date()"
                                            class="calender_bts date_calender_btn btn btn-primary casf active">Date</button>
                                        <button type="button" onclick="open_month()"
                                            class="calender_bts month_calender_btn btn btn-primary casf">Month</button>
                                        <button type="button" onclick="open_year()"
                                            class="calender_bts year_calender_btn btn btn-primary casf">Year</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="setdatediv" class="all_calender_panel_div">
                            <div class="tupdata">
                                ...
                                <i class="fa fa-arrow-right aroowm" aria-hidden="true"
                                    onclick="new_month_panel('next')"></i>&nbsp;&nbsp;
                                <i class="fa fa-arrow-left aroowm" aria-hidden="true"
                                    onclick="new_month_panel('previous')"></i>
                            </div>
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="topdays">
                                ...
                                <div class="row">
                                    <div class="col-sm-12 dateboxarea">
                                        @foreach ($months as $key_id => $month)
                                            <div class="datebo month_bg month_div_{!! $key_id !!}"
                                                onclick="set_month('{!! $key_id !!}')">
                                                <p>{!! $month !!}</p>
                                            </div>
                                        @endforeach
                                    </div>
                        ...
                        <div id="setyeardiv" class="all_calender_panel_div">
                            <div class="tupdata">
                                ...
                                <i class="fa fa-arrow-right aroowm" aria-hidden="true"
                                    onclick="new_year_panel('next')"></i>
                                &nbsp;&nbsp;<i class="fa fa-arrow-left aroowm" aria-hidden="true"
                                    onclick="new_year_panel('previous')"></i>
                            </div>
                            <div class="calender_year_panel"></div>
                        </div>
...
<script>
    ...
    function create_month_list(month) {
        ...
        for (var i = 0; i < 5; i++) {
            ...
            for (date; date < end_date; date++) {
                ...
                month_html = month_html + `
                                <div class="datebo dtebo date_bg date_div_` + date_data + `" onclick="set_date(` +
                    date_data + `)">
                                    <p>` + date_data + `</p>
                                </div>
                `
    ...
    function date_exist(date) {
        ...
        var index = selected_calender_dates.findIndex(x => x.date == date.date && x.month == date.month && x.year ==
            date.year);
        return index;
    }
    ...
    function set_date(date) {
        ...
        var index = date_exist(date_obj);
        console.log('index', index);

        if (index === -1) {
            selected_calender_dates.push(
                date_obj
            );
        } else {
            console.log('index 2', index);
            selected_calender_dates.pop(index);
        }
        console.log('date_obj', date_obj);
        console.log('alldates', selected_calender_dates);
        highlight_selected_dates();
        $(date_input).val(JSON.stringify(selected_calender_dates));

    }

    function highlight_selected_dates() {
        $('.date_bg').removeClass('selected_date');
        for (var i = 0; i < selected_calender_dates.length; i++) {
            if (calender_month == selected_calender_dates[i].month && calender_year == selected_calender_dates[i]
                .year) {
                $('.date_div_' + selected_calender_dates[i].date).addClass('selected_date');
            }
        }
    }
